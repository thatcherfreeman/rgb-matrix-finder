...
ref_chart, _ = load_reference_chart(read_text_file(os.path.join("reference_charts", "ColorChecker24_After_Nov2014.txt")))
src_chart: RGBChart = ref_chart.convert_to_xyz(ref_chart.reference_white) \
                        .convert_to_xyz(ColorMatrix.get_chromatic_adaptation_matrix(ref_chart.reference_white, src_gamut.white.convert_to_xyz())) \
                        .convert_to_rgb(src_gamut.get_conversion_to_xyz().inverse())
src_chart.colors += np.random.normal(0.0, 0.0001, size=src_chart.colors.shape)


def test_optimize() -> None:
    parameters: Parameters = optimize(src_chart, ref_chart, target_gamut, True)
    assert np.sum(np.abs(parameters.matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.01
...
def test_optimize_exp() -> None:
    ...
    parameters: Parameters = optimize_exp(src_chart2, ref_chart, src_gamut, target_gamut, True)
    assert np.abs(parameters.exposure - (1.0 / exp)) < 0.0001
    assert np.sum(np.abs(parameters.matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.0001
...
def test_optimize_exp_wb() -> None:
    ...
    parameters: Parameters = optimize_exp_wb(src_chart2, ref_chart, src_gamut, target_gamut, True)
    assert np.sum(np.abs(parameters.matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.001
...
def test_optimize_nd_exp() -> None:
    ...
    assert np.sum(np.abs(parameters[0].matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.0001
...
def test_optimize_nd_exp_wb() -> None:
    ...
    assert np.sum(np.abs(parameters[0].matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.0001
    assert np.abs(parameters[0].white_balance.mat[0, 0] - (1.0 / wb_coeffs1[0, 0])) < 0.01
    assert np.abs(parameters[0].white_balance.mat[2, 2] - (1.0 / wb_coeffs1[2, 2])) < 0.01
    assert np.abs(parameters[1].white_balance.mat[0, 0] - (1.0 / wb_coeffs2[0, 0])) < 0.01
    assert np.abs(parameters[1].white_balance.mat[2, 2] - (1.0 / wb_coeffs2[2, 2])) < 0.01
...
def test_optimize_nd() -> None:
    ...
    parameters: List[Parameters] = optimize_nd(
        ...
        target_gamut,
        True,
        [Parameters(), Parameters(ColorMatrix(np.eye(3), ImageState.RGB, ImageState.RGB))]
    )
    assert np.abs(parameters[0].exposure - (1.0 / exp1)) < 0.01
    assert np.abs(parameters[1].exposure - (1.0 / exp2)) < 0.01
    assert np.sum(np.abs(parameters[0].matrix.mat - (src_gamut.get_conversion_to_gamut(target_gamut).mat))) < 0.001
    assert np.abs(parameters[0].white_balance.mat[0, 0] - (1.0 / wb_coeffs1[0, 0])) < 0.01
    assert np.abs(parameters[0].white_balance.mat[2, 2] - (1.0 / wb_coeffs1[2, 2])) < 0.01
    assert np.abs(parameters[1].white_balance.mat[0, 0] - (1.0 / wb_coeffs2[0, 0])) < 0.01
    assert np.abs(parameters[1].white_balance.mat[2, 2] - (1.0 / wb_coeffs2[2, 2])) < 0.01
